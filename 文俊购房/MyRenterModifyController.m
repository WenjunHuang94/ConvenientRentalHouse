...
#import "MyRenterModifyController.h"
...
@interface MyRenterModifyController ()<UITableViewDelegate,UITableViewDataSource,SelectCityViewControllerDelegate,UITextViewDelegate>
...
@implementation MyRenterModifyController
...
- (void)viewDidLoad{
    ...
    //设置标题
    [self setNavTitle:@"我的求租编辑" AndImg:nil];
    [self setNavLeftBtnWithTitle:nil OrImage:@[@"返回"]];
    
    [self createView];
    
    __weak MyRenterModifyController *obj = self;
    ...
    if ([self check]) {//检查
        ...
        [home saveInBackgroundWithBlock:^(BOOL succeeded, NSError * _Nullable error) {
            if (succeeded) {
                ...
                if ([self.delegate respondsToSelector:@selector(UpdateModify:)]) {
                    [self.delegate UpdateModify:self];
                }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ((indexPath.section == 0) && (indexPath.row == 0)) {
        ...
        if (!self.cityCell) {
            ...
            self.cityCell.tipField.text = [self.object objectForKey:@"city"];
        }
    ...
    //小区
        ...
        if (!self.districtCell) {
            ...
            self.districtCell.tipField.text = [self.object objectForKey:@"district"];
        }
    ...
    //房名
        ...
        if (!self.homeNameCell) {
            ...
            self.homeNameCell.tipField.text = [self.object objectForKey:@"homeName"];
        }
    ...
    //详细地址
        ...
        if (!self.AreaDetailCell) {
            ...
            self.AreaDetailCell.tipField.text = [self.object objectForKey:@"areaDetail"];
        }
    ...
    //房型
        ...
        if (!self.HomeTypeCell) {
            ...
            NSString *homeType = [self.object objectForKey:@"homeType"];
            NSRange homeRange = [homeType rangeOfString:@"房"];
            NSRange hallRange = [homeType rangeOfString:@"厅"];
            NSRange toiletRange = [homeType rangeOfString:@"卫"];
            
            self.HomeTypeCell.roomField.text = [homeType substringWithRange:NSMakeRange(0, homeRange.location - 0)];//起点,长度
            
            self.HomeTypeCell.hallField.text = [homeType substringWithRange:NSMakeRange(homeRange.location + 1,hallRange.location - (homeRange.location + 1))];
            
            self.HomeTypeCell.toiletField.text = [homeType substringWithRange:NSMakeRange(hallRange.location + 1,toiletRange.location - (hallRange.location + 1))];
        }
    ...
    //面积
        ...
        if (!self.areaCell) {
            ...
            NSNumber *area = [self.object objectForKey:@"area"];
            self.areaCell.contentField.text = [NSString stringWithFormat:@"%@",area];
    ...
    //楼层
        ...
        if (!self.floorCell) {
            ...
            self.floorCell.tipField.text = [self.object objectForKey:@"floor"];
        }
    ...
    //租金
        ...
        if (!self.rentMoneyCell) {
            ...
            NSNumber *rentMoney = [self.object objectForKey:@"rentMoney"];
            self.rentMoneyCell.contentField.text = [NSString stringWithFormat:@"%@",rentMoney];
        }
    ...
    //租期
        ...
        if (!self.timeCell) {
            ...
            self.timeCell.tipField.text = [self.object objectForKey:@"time"];
        }
    ...
    //入住时间
        ...
        if (!self.startTimeCell) {
            ...
            self.startTimeCell.tipField.text = [self.object objectForKey:@"startTime"];
        }
    ...
    //类型
        ...
        if (!self.homeUseTypeCell) {
            ...
            self.homeUseTypeCell.tipField.text = [self.object objectForKey:@"homeUseType"];
        }
        ...
        if (!self.requestCell) {
            ...
            self.requestCell.inputView.text = [self.object objectForKey:@"require"];
        }
...
-(void)BtnClick:(UIButton *)btn{//点击事件
