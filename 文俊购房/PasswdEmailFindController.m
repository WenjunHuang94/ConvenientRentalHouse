...
#import "PasswdEmailFindController.h"

@interface PasswdEmailFindController ()

@property (nonatomic,strong)UIActivityIndicatorView *activeView;//数据加载动画

@end

@implementation PasswdEmailFindController{
...
-(void)viewDidLoad {
    ...
    [self setNavTitle:@"邮箱找回密码" AndImg:nil];
    [self setNavLeftBtnWithTitle:nil OrImage:@[@"返回"]];
    
    
    //点击登录按钮时，返回上一个界面
    
    __weak PasswdEmailFindController *obj = self;
...
-(void)createView{
    ...
    codeField.keyboardType = UIKeyboardTypeDefault;//键盘格式
...
-(void)confirmBtnClick{
    ...
    if ([codeField.text isEqualToString:@""]) {
        ...
        //发送邮箱进行密码修改
        [AVUser requestPasswordResetForEmailInBackground:codeField.text block:^(BOOL succeeded, NSError *error) {
            if (succeeded) {
                [Tip myTip:@"发送邮件成功!"];
            }
